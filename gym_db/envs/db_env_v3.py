...
class DBEnvV3(gym.Env):
    ...
    def _step_asserts(self, action):
        ...
        if self.environment_type != EnvironmentType.TRAINING and self.MAX_INDEXES is not None:
            assert (
                len(self.current_indexes) < self.MAX_INDEXES
            ), f"More indexes than allowed: {len(self.current_indexes)} >= {self.MAX_INDEXES}"
    ...
    def step(self, action):
        ...
        if self.current_budget:
            if (
                b_to_mb(self.current_storage_consumption + self.action_manager.action_storage_consumptions[action])
                > self.current_budget
            ):
                self.valid_actions, is_valid_action_left = self.action_manager.update_valid_actions(
                    action, budget=None, current_storage_consumption=None
                )
                if not is_valid_action_left:
                    self._report_episode_performance(self.last_environment_state)
                    self.current_workload_idx += 1
                    return self.last_observation, 0, True, {"action_mask": self.valid_actions}
                return self.last_observation, 0, False, {"action_mask": self.valid_actions}

        self.current_indexes.add(new_index)
        self.current_indexes_creation_order.append(new_index)

        environment_state = self._update_return_env_state(init=False, new_index=new_index)
        ...
        if self.environment_type != EnvironmentType.TRAINING and self.MAX_INDEXES is not None:
            episode_done = episode_done or len(self.current_indexes) == self.MAX_INDEXES
    ...
    def _report_episode_performance(self, environment_state):
        episode_performance = {
            ...
            "indexes_creation_order": self.current_indexes_creation_order,
        }
        ...
        output = (
            ...
            f"{self.current_indexes_creation_order}\n    "
        )
    ...
    def _init_modifiable_state(self):
        ...
        self.current_indexes_creation_order = []
        self.steps_taken = 0
        ...
        if self.environment_type == EnvironmentType.TRAINING:
            ...
            assert self.current_workload.budget is None
    ...
    def _update_return_env_state(self, init, new_index=None):
