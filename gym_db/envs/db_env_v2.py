...
class DBEnvV2(gym.Env):
    ...
    def step(self, action):
        ...
        if new_index in self.current_indexes:
            environment_state = self._update_return_env_state(init=False, new_index=None, old_index_size=old_index_size)
            ...
            return current_observation, -1, True, {"action_mask": self.valid_actions}
        ...
        if self.current_budget is not None and b_to_mb(self.current_storage_consumption) > self.current_budget:
            if self.environment_type != EnvironmentType.TRAINING:
                self._report_episode_performance(environment_state)
                self.current_workload_idx += 1
            return current_observation, -1, True, {"action_mask": self.valid_actions}
        ...
        assert np.count_nonzero(np.array(self.valid_actions) == self.action_manager.ALLOWED_ACTION) == len(
            self.valid_actions
        ), "All actions must remain valid."
        assert is_valid_action_left is True, self.valid_actions
